...
async function backup_a_src_dir(rootdir_src, rootdir_target) {
    ...
    for (let i = 0; i < keys_srcfiles.length; i++) {
        ...
        // check if file exist in the target folder
        let targetFileExist = fs.existsSync(thetargetfilename_withpath)

        if (targetFileExist) { // if the file (corresponding to the src file) can be found in the target_dict
...
async function get_files_subdirs(thedir) {
    ...
    if (dirExist) {
        ...
        console.log(134, 'number of file and subdirs:', names_arr.length)
