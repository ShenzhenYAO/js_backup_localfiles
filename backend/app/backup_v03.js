...
(async () => {
    ...
    console.log('no need at all ! the windows xcopy cmd is good enough for backup. Check out backup.bat!')

})()

// ************************************************************************************
// the following is a long version of the same as the above (with detail explanations)
...
async function get_offspringdirs(thedir) {
    thedir = thedir.replace(/\\/g, '/')
    let offspringdirs = await Promise.all(
        fs.readdirSync(thedir, { withFileTypes: true }) // get an Array of Dirend objects with entry (a file or a dir) names and type
            .filter(x => x.isDirectory()) // filter (selete) the directories
            .map(async (x) => (thedir + '/' + x.name.replace(/\\/g, '/')))
        ...
        return d.concat(
            await Promise.all(d.map(async (x) => (await get_offspringdirs(x))))
                .then(async (e) => (e.reduce((a, b) => (a.concat(b)), [])))
        ) // d.concat(
    }) // then async(d)

    return offspringdirs
} // async function get_offspringdirs
